...
const activeWorkbench = ref<'research' | 'master' | 'visuals' | 'gallery' | 'variants' | 'approval'>('master')
...
const { wechat: wechatSettings } = storeToRefs(settingsStore)
...
const wechatPreflight = computed(() => {
  ...
  if (approvalStatus.value?.stale) issues.push('审批已过期，请重新检查')
  if (!approvalStatus.value?.complete) issues.push('内容包尚未完成审批')
  return issues
})
const officialCapabilities = computed(() => capabilitiesStore.items.filter(item => item.official_api && item.auth?.kind === 'oauth_user'))
...
function goPrimaryCta(): void {
  const key = primaryCta.value.key as typeof activeWorkbench.value
  activeWorkbench.value = key
  extraPanels.value = key === 'master' ? [] : [key]
}
...
function setupStandardVisuals(): void {
  ...
  for (const slot of visualSlots.value) visualPrompts.value[slot.id] = slot.direction
}
...
async function generateVisual(slot: VisualSlot, referenceAssetId?: string): Promise<void> {
  const prompt = visualPrompts.value[slot.id]?.trim() || slot.direction
  if (!projectId.value || !prompt) return
  if (!llmAllowed.value) { detailError.value = llmForbiddenHint(project.value?.autonomy); return }
  visualGenerating.value = slot.id
  try {
    if (referenceAssetId) await visualsStore.edit(projectId.value, slot.id, prompt, referenceAssetId)
    else await visualsStore.generate(projectId.value, slot.id, prompt)
  } catch (e) {
    detailError.value = unwrapError(e)
    await visualsStore.load(projectId.value)
  } finally { visualGenerating.value = null }
}
...
function markdownBundle(title: string, body: string): string {
  return `# ${title}\n\n${body}`
}
async function copyMarkdown(title: string, body: string): Promise<void> {
  await navigator.clipboard.writeText(markdownBundle(title, body))
}
function downloadMarkdown(filename: string, title: string, body: string): void {
  const blob = new Blob([markdownBundle(title, body)], { type: 'text/markdown;charset=utf-8' })
...
async function createWechatDraft(): Promise<void> {
  ...
  drafting.value = true
  try {
    draftResult.value = await approvalsStore.createWechatDraft(projectId.value, actor, lastFailedDraftId.value, boundWechatId.value)
  ...
  <section class="projects-page">
    <template v-if="projectId">
      ...
      <article v-if="project" class="project-workspace">
        ...
        <nav class="article-toolbar" aria-label="作品操作">
          <a-button type="primary" data-testid="project-primary-cta" @click="goPrimaryCta">{{ primaryCta.label }}</a-button>
          <p class="muted">正文始终在中间。资料、配图、平台稿和交付按需展开，不会盖住文章。</p>
        </nav>
        <section class="master-workbench article-center" data-testid="article-center">
        ...
        <a-collapse v-model:activeKey="extraPanels" class="extra-panels">
          ...
          <a-collapse-panel key="visuals" header="配图">
        ...
        <section class="gallery-workbench">
          ...
          <a-collapse-panel key="variants" header="微信与头条稿">
        <section class="variants-workbench">
        ...
        <section class="approval-workbench">
          ...
          <div v-if="longtailCapabilities.length" class="export-panel official-caps"><div><strong>长尾平台</strong><p>无官方创作者发布 API。只提供安全导出或「暂不支持官方发布」，不会标成可直发。</p></div><ul class="capability-list"><li v-for="item in longtailCapabilities" :key="item.platform"><strong>{{ item.label }}</strong> · {{ item.formats.join(' / ') }} · {{ item.lane === 'export' ? '仅导出' : item.lane }} · 官方 API 无<p>{{ item.ui.confirm_copy }}</p></li></ul></div>
        </section>
          </a-collapse-panel>
        </a-collapse>
        <a-modal v-if="previewAsset" :open="Boolean(previewAsset)" :footer="null" :title="previewAsset.status === 'selected' ? '已选择候选' : '候选预览'" @cancel="previewAsset = null">
          <img v-if="visualAssetUrl(previewAsset)" :src="visualAssetUrl(previewAsset) || ''" :alt="previewAsset.prompt" style="width: 100%" />
          <p class="muted">{{ previewAsset.prompt }}</p>
        </a-modal>
      </article>
    </template>
...
<style scoped>
.projects-page { max-width: 1180px; padding: 24px 0 56px; }
