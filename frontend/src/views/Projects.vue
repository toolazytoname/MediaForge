...
const workflowSteps = computed(() => [
  { key: 'master' as const, label: '文章', done: masterReady.value },
  { key: 'variants' as const, label: '微信稿', done: wechatReady.value },
  { key: 'visuals' as const, label: '配图', done: visualsReady.value },
    ...
    <template v-if="projectId">
      ...
      <article v-else-if="project" class="project-workspace">
        ...
        <section v-show="activeWorkbench === 'variants'" class="variants-workbench">
          <header class="section-heading"><div><p class="eyebrow">微信公众号</p><h2>先把这篇当成读者会打开的文章。</h2><p>左侧是阅读态。标题、摘要和正文仍可独立修改；主稿更新只会提示，不会覆盖你改过的微信稿。</p></div></header>
...
@media (max-width: 760px) { .list-header, .project-row { align-items: flex-start; flex-direction: column; }.project-grid, .research-grid, .form-pair, .variant-list, .workflow-cockpit, .wechat-stage, .wechat-layout { grid-template-columns: 1fr; }.workflow-cockpit { position: static; }.workflow-steps { grid-template-columns: repeat(5, minmax(110px, 1fr)); overflow-x: auto; }.row-meta { width: 100%; justify-content: space-between; } }
</style>
