...
async function createVariant(platform: PlatformVariant['platform']): Promise<void> { if (!projectId.value) return; try { const item = await variantsStore.create(projectId.value, platform); variantForms.value[item.platform] = { title: item.title, summary: item.summary, body: item.body } } catch (e) { detailError.value = unwrapError(e) } }
async function saveVariant(item: PlatformVariant): Promise<void> { if (!projectId.value) return; try { const form = variantForm(item.platform); await variantsStore.save(projectId.value, item.platform, { ...form, asset_ids: item.asset_ids }) } catch (e) { detailError.value = unwrapError(e) } }
async function toggleLock(item: PlatformVariant): Promise<void> { if (!projectId.value) return; try { await variantsStore.lock(projectId.value, item.platform, !item.locked) } catch (e) { detailError.value = unwrapError(e) } }
