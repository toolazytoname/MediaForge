    ...
    <template v-if="projectId">
      ...
      <article v-else-if="project" class="project-workspace">
        ...
        <section v-show="activeWorkbench === 'approval'" class="approval-workbench">
          ...
          <div v-if="officialCapabilities.length" class="export-panel official-caps"><div><strong>官方平台能力</strong><p>按 CapabilityRegistry 渲染。无用户授权时直发关闭。Project 不提供一键公开直发。</p></div><ul class="capability-list"><li v-for="item in officialCapabilities" :key="item.platform"><strong>{{ item.label }}</strong> · {{ item.formats.join(' / ') }} · 授权 {{ item.auth?.kind ?? 'oauth' }} · {{ item.review?.requires_app_review ? '需应用审核' : '无需应用审核' }} · 默认可视性 {{ item.review?.default_visibility ?? 'unknown' }} · 有效直发 {{ item.delivery_effective?.direct ? '开' : '关' }}<p>{{ item.ui.confirm_copy }}</p></li></ul></div>
        </section>
      </article>
    </template>
