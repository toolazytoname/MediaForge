  ...
  <main class="creator-prototype" aria-label="MediaForge 文章优先原型">
    ...
    <section v-else-if="screen === 'article'" class="article-view">
      ...
      <article class="article-paper">
        ...
        <p class="selectable" :class="{ selected: selectedText }" @click="selectedText = true">可真正坐到电脑前时，我还是停在了第一步：我有一个想法，但不知道该点哪一个按钮。原来我搭建的不是一张工作台，而是一条需要先理解说明书的流水线。</p>
        <div v-if="selectedText" class="inline-comment" role="dialog" aria-label="针对段落提出意见">
          <span>已选中这一段</span>
          <a-textarea v-model:value="comment" :auto-size="{ minRows: 2, maxRows: 4 }" />
