...
> **G1 当前进度（2026-08-12）**：`PF-01`（commit `e0366ef`）已将首页接到真实单输入项目创建；`PF-02`（commit `246f551`）已提供项目侧的 URL、笔记、图片、PDF、Markdown 资料入口与独立失败状态；`PF-03`（commit `f84b69c`）已提供严格、可追溯的资料解析和安全 URL 读取。下一项是 `PF-04` 的一次显式生成完整图文文章；不得把“资料已添加”或“解析失败”伪称为已经使用。
