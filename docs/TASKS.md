...
> 2026-08-12：产品所有者已确认 `UX-00` 的文章优先信息架构，并明确确认“选中即评论 / 右键评论”局部批注交互；根路由直接进入“主题 + 想法 + 可选资料 → 文章”的路径。`PF-00` 工程资产审计也已完成，现进入 G1 的真实实现。
...
> **G1 当前进度（2026-08-12）**：`PF-01`（commit `e0366ef`）已将首页接到真实单输入项目创建；`PF-02`（commit `246f551`）已提供项目侧的 URL、笔记、图片、PDF、Markdown 资料入口与独立失败状态；`PF-03`（commit `f84b69c`）已提供严格、可追溯的资料解析和安全 URL 读取；`PF-04` 的工程实现（commit `f915ce6`）已落地可恢复的正文/封面/两张插图生成与文章工作区，兼容图片模型设置见 `0bdd3ea`。真实文字已在浏览器验证；默认图片模型被中转站返回 `model_not_found`，尚待用户提供支持的模型名完成真实图文验收，任务保持未勾选。不得把“资料已添加”“解析失败”或实现提交伪称为已经生成。

> **PF-05 已完成（2026-08-12，commit `8c8fd15`）**：`/projects/<id>` 默认为文章本体，资料/版本降到抽屉；真实浏览器已验证编辑、保存、刷新、版本恢复展示，以及图片失败不影响正文。下一步仍是补齐 PF-04 的真实封面/两张插图验收（需要中转站支持的模型名），随后进入 G2 的人工批注与提案链路。

> **RV-01 已完成（2026-08-12，commit `0e4e239`）**：整篇意见被记录为只读审阅提案并绑定主稿版本/内容指纹；真实浏览器已验证 AI 提案生成后正式文章完全不变。下一步为 `RV-02` 的文本/图片局部批注，随后才实现 diff 与明确确认。

> **RV-02 工程实现已完成（2026-08-12，commit `c4a5296`）**：文本与图片批注的严格 sidecar、重定位/失配保护、选区和右键 UI 已落地；专项测试/构建通过。远程浏览器拖选未能产生原生选区，暂未取得真人选区操作证据，任务保持未勾选；不得把 API 或源码当作该交互验收。

> **RV-03 已完成（2026-08-12，commit `dea590c`）**：真实提案可在双栏与逐行差异中审阅；真人浏览器已验证拒绝不改正式文章。接受会做版本/内容指纹检查、写新主稿版本，并在中断后从主稿历史恢复审计记录。

> **RV-04 已完成（2026-08-12，commit `3fa6951`）**：接受/恢复都有明确确认和追加版本语义，隔离项目真人验证恢复成功。验收中曾误在 `prj_1ff11754` 执行恢复，随后已恢复回操作前 v3 正文；当前 v5 内容与 v3 相同，中间 v4/v5 作为不可变审计历史保留，不得删除或伪装未发生。

> **RV-05 工程实现已完成（2026-08-12，commit `4f45076`）**：文章内换图现为候选比较后显式选择；只替换当前图片 Markdown 引用、产生新主稿版本并保留原图/候选/失败审计。专项 33 项与构建通过。任务仍未勾选：中转站默认 `gpt-image-2` 返回 `model_not_found`，尚未在浏览器针对真实图片完成“生成候选、拒绝、选择另一个、恢复原图”的验收。另经复核，RV-03 当前仅把**整篇**意见接入提案；局部批注进入同一 diff/接受闭环的修复正在进行，G2 不得宣称通过。

> **RV-03L 已完成（2026-08-12，commit `033e400`）**：上述局部提案缺口已修复：作者对局部文字或图片批注点击“生成修改提案”后，会生成同一套可编辑双栏 diff；proposal sidecar 同时冻结批注 ID/种类/摘录或图片锚点/类别/已解析版本和指纹，孤儿或过期批注被硬阻断。接受、拒绝与失败重试沿用 RV-03 的意图先行和版本审计。专项 22 项与构建通过；仍需 RV-02 的真实选区浏览器验收，G2 未通过。

> **RV-03L 对抗修复已完成（2026-08-12，commit `896ce64`）**：审查发现失败的局部提案重试曾错误退回整篇文章 prompt，丢失选区/图片锚点与分类。现按 scope 重新解析批注、使用局部 prompt、在写回前二次校验文章与锚点，并与首次生成共用并发锁；批注失配返回可操作的 `annotation_obsolete`，不会生成错误范围的建议。相关专项 23 项通过。

> **图片模型确认辅助已完成（2026-08-12，commit `8db549a`）**：设置页新增只读“检测可用模型”，安全请求已配置中转站的兼容 `/models`；只返回合规模型标识，不回显中转地址、密钥或上游错误内容，不生成图片且不写入当前模型。当前用户中转站实测返回 0 个模型，因此仍不能推断或自动选择图片模型；需服务商提供 Images API 实际支持的模型名后才能进行真实图片验收。
...
> 当前唯一实现优先级是[创作者真人验收与整改报告](./product-validation/2026-08-10-creator-workflow-remediation.md)的 P0-A 至 P0-G。每项都必须在浏览器完成对应真人路径；不得进入 R10。

- [x] **P0-A｜构建更新可恢复**：SPA `index.html` 强制重新验证，hash 静态资源长期缓存；动态路由 chunk 失败自动刷新一次，仍失败时显示可见刷新操作。后端缓存契约测试与生产构建通过，浏览器已实际打开更新后的入口并走过路由入口。

### R0｜固定一条真实主题验收剧本（文档任务，先于 R1）

- [ ] **目标**：把“我想要一个好用的创作工作台”收敛成一条本周真实会发布的、可重复观察的验收路径。它是 R1—R9 的共同产品验收样本，而不是示例内容或自动化假数据。
- **输入**（由用户提供或明确确认，缺一项即记录阻塞而不虚构）：
  1. 一个真实主题，以及 3—5 个可靠来源（URL、笔记或文件）；
  2. 目标读者、发布目的和作者的独特观点/经验；
  3. 主稿质量标准：核心主张、事实边界、预期篇幅、语气与不可接受的内容；
  4. 两个首发目标平台：微信公众号图文与今日头条图文；
  5. 一个真实可用账号或安全导出方式。R0 不触发真实发布。
- **步骤 / 验收剧本**：
  1. 用一句话记录主题、目标读者和读者读完后应获得的具体价值；
  2. 登记来源与自己的观点，区分“可核查事实、个人判断、待确认项”，并指定至少一条反方或限制条件；
  3. 在 60 分钟计时内，从空白或项目入口完成一篇主稿，记录每次离开系统、卡住、等待或手工修复的原因与时长；
  4. 生成或手工整理一张封面和两张与正文相对应的插图，记录视觉意图、prompt/来源、候选取舍及人工修改；
  5. 基于同一主稿得到微信公众号和头条两个版本；逐个检查标题、正文层级、图片、引用/事实、平台限制及人工修改，确认任一版本的修改不会覆盖另一个版本；
  6. 只走“平台草稿箱或安全导出 / dry-run”路径，记录可见回执、失败信息和恢复步骤，明确标注未真实发布；
  7. 结束时填写总耗时、人工修改比例、离开工作台的次数、愿不愿署名发布，以及下一步最应消除的三个摩擦点。
- **测试 / 验证**：
  - 验收记录必须含以上输入、时间线、主稿与两个平台版本的链接/路径、视觉资产清单、检查清单与观察结论；不得以 mock、占位内容或“页面能打开”代替真人路径；
  - 在浏览器中亲自走完该剧本至少一次；验收者能从记录复现每一步并定位任何离开系统的动作；
  - 通过标准：主题、读者、质量标准和两个平台明确；全程可审计；结尾明确 R1—R9 应解决的摩擦点。是否真实发布不属于 R0 通过条件。
- **明确不做**：不改代码、前端、配置、数据库或既有任务状态；不扩展小红书、视频号、Bilibili、数字人、多账号或无人值守发布；不接入 GPT Image 2；不为了完成剧本而伪造来源、账号、指标或发布回执。
...
- [x] **目标**：把普通用户的主导航从旧流水线阶段收敛为“今天 / 灵感 / 项目 / 资产 / 发布 / 复盘”，使首页成为可继续工作的安静工作台，而非紫色横幅和状态统计页。
- **步骤**：
...
- **声明改动文件**：`docs/TASKS.md`、`pipeline/webui/api/__init__.py`、`pipeline/webui/api/projects.py`、`tests/webui/test_projects_api.py`、`frontend/src/router/index.ts`、`frontend/src/layouts/AppShell.vue`、`frontend/src/views/Today.vue`、`frontend/src/views/Projects.vue`、`frontend/src/stores/index.ts`、`frontend/src/App.vue`、`frontend/dist/`（仅由 `npm run build` 再生成）。既有用户脏文件 `CLAUDE.md`、`frontend/src/views/PublishCenter.vue`、`pipeline/publishers/multipost_bridge.py` 与 `state.db.bak-20260711-090813` 不纳入本任务 diff gate。
...
- [x] **目标**：让每个 Project 在写稿前拥有可编辑、可追溯的研究板，明确区分来源事实、个人判断与待确认项，并让冲突或反方限制显式可见。
- **步骤**：
...
- **步骤**：
  ...
  3. 提供只读的微信 / 头条预览 HTML，展示标题、摘要、正文、已选择视觉资产及平台差异说明；预览不调用发布器、不创建 Publication、不请求网络；
  4. 项目工作台新增“平台版本”区域：用户可创建两份初稿、独立编辑/锁定、看到上游版本提示、预览微信与头条排版，并清楚看到“不会自动覆盖你的修改”。
- **测试 / 验收**：
  - 覆盖 sidecar round-trip、严格字段与资产引用、原子写、双平台限定、创建幂等、独立手工修改、锁定、上游更新、恢复语义与坏 manifest；
  - API 覆盖错误 envelope、预览只读、禁止创建其他平台、无 SQLite/Content/Publication 变化；
  - 浏览器真人路径：先保存主稿，创建微信与头条版本，手改并锁定其中一份，再更新主稿，确认只显示上游更新，两个预览均可查看且未触发发布；
