...
- **红线**：不得对 `prj_a63f79b2` 或未授权内容做公开发布实测。无授权则标 BLOCKED 并继续其他任务。
  ...
  ✅ 复审修复 2026-09-10：草稿转直发在调用 `adapter.publish()` 前取得文件锁；远端 media_id 必须匹配当前审批版本/指纹，否则 `draft_stale`；非终态核对使用带时间戳的幂等键，允许再次查询平台。作品页首次展示账号即写入绑定。
...
- **声明改动文件**：本文件、`docs/TASKS.md` 完成摘要、走查证据（新文件，不含保留的 open-*.png）。
  ...
  ⚠️ 2026-09-10 评审退回：上次“全部完成”不成立。第二次复审 8 项 P1 已修代码与隔离测试，全量 `1926 passed`，生产构建通过；QA-01 仍不勾选，待作者走查与真实回执核定。未对 `prj_a63f79b2` 公开发布。
