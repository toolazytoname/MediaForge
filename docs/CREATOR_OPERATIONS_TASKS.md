...
- **红线**：不改 Project frozen 字段；不把 secret 写入 sidecar；不开启现有账号运营。
  ...
  ✅ 复审修复 2026-09-10：作品页无绑定时不再只显示第一个公众号；会立即写入绑定，避免页面看似已选但发送被拒。
  ✅ 复审修复 2026-09-10：撤销自动绑定。未绑定必须用户明确选择；读取绑定失败显示错误，不把失败当成未绑定。

---

## UX-03 参考资料访谈 → 账号定位

- [x] **目标**：用户提供参考文章/可读取链接 + 简短访谈后，生成账号定位与风格建议，经用户确认才写入档案。
...
- **红线**：不静默覆盖已确认档案；不把建议标成已确认。
  ...
  ✅ 完成于 2026-09-10，全量 `1870 passed`。建议 sidecar 未确认不写档案；失败 URL 记入 unread_sources。

---

## UX-04 简化导航与真实待办首页

- [x] **目标**：主导航收敛为今天 / 作品 / 账号；明显开始创作与设置；首页接真实待办/异常/下次计划。旧路由保持兼容。
...
- **红线**：不删除旧路由文件；开发者抽屉仍可进运行状态，但不是日常入口。
  ...
  ✅ 完成于 2026-09-10。主导航今天/作品/账号；Today 读 `/api/v1/today`；webui+today 319 passed。

---

## UX-05 个人创作先访谈后写稿

- [x] **目标**：个人稿在写正文前先访谈并保存作者观点和经历；只有书名时不能假装读过原书。
...
- **红线**：不编造个人经历；AI 稿仍是建议，不自动覆盖现稿。
  ...
  ✅ 完成于 2026-09-10。无确认访谈起草返回 `interview_required`；书名无摘录写入 unread_books 并进入 prompt。webui 相关 287 passed。

---

## UX-06 文章居中工作台

- [x] **目标**：文章居中，AI 建议在侧边，先审阅后接受；版本可恢复；微信和头条稿独立编辑。
...
- **红线**：不静默应用 AI 建议。
  ...
  ✅ 完成于 2026-09-10。打开作品即见主稿；资料/配图/平台稿/交付用折叠面板。主稿建议仍需接受才写入。桌面/手机截图 `docs/product-validation/ux06-article-center*.png`。
...
- **红线**：不伪造审批记录；不把机器质量分写成 human_verified。
  ...
  ✅ 复审修复 2026-09-10：自动授权要求 `verdict=pass`、正文指纹与当前授权版本一致；改稿后不得沿用旧质量结果。
  ✅ 复审修复 2026-09-10：质量指纹覆盖主稿和平台稿；单独改写公众号版本会使旧质量结果失效。
...
- **红线**：不得对 `prj_a63f79b2` 或未授权内容做公开发布实测。无授权则标 BLOCKED 并继续其他任务。
  ...
  ✅ 复审修复 2026-09-10：草稿转直发在调用 `adapter.publish()` 前取得文件锁；远端 media_id 必须匹配当前审批版本/指纹，否则 `draft_stale`；非终态核对使用带时间戳的幂等键，允许再次查询平台。
  ✅ 复审修复 2026-09-10：外部调用前写入发送意图（unknown）；中断后再次请求只恢复/查询，不重发。未绑定作品不再自动写入第一个公众号；读取绑定失败会报错而不是当成未绑定。
...
- [x] **目标**：替换 `prepare_pack` 占位正文；真实选题去重、研究、写稿、封面插图、平台适配；最多自动修订两轮，不合格暂停。
...
- **红线**：自动路径仍不得 `safe_publish(dry_run=False)`，除非 DEL-01 授权且任务要求交付。
  ...
  ✅ 复审修复 2026-09-10：默认配图调用 `generate_image` 并选中资产；运营任务抓取参考链接正文写入研究摘要，读失败标明未读。
  ✅ 复审修复 2026-09-10：配图按槽位补齐，缺图不算完成；`generate_image` 传入数据库连接做审计和预算，资产费用不再写 0，并累计账号消费。
...
- **红线**：不改 publications UNIQUE 语义；结果未知不重试发送。
  ...
  ✅ 复审修复 2026-09-10：任务写入 `scheduled_at`，未到期不执行；关闭账号后已排队任务保持 queued；交付失败不得把任务标 `done`。
  ✅ 复审修复 2026-09-10：Web UI 启动后每 60 秒 tick 到期任务；`python -m pipeline.run ops --loop` 可独立循环。排期选出不同主题后再去重。running 任务从 `result_path` 恢复已创建项目，不另起一篇。

---

## UX-07 异常回到今天

- [x] **目标**：失败、缺资料、登录过期、质量不合格出现在今天，并提供补资料、编辑、重试、跳过、核对结果。
...
- **声明改动文件**：待办模块、Today 页、测试、本文件。
  ...
  ✅ 复审修复 2026-09-10：发布中回执再次核对会重新查询平台，终态成功/失败才缓存。
...
- **声明改动文件**：本文件、`docs/TASKS.md` 完成摘要、走查证据（新文件，不含保留的 open-*.png）。
  ...
  ⚠️ 2026-09-10：上次“全部完成”不成立。第四次复审的五项缺陷已修复并提交，详见 REV-04；最新分批全量回归 **1950 passed**，生产构建通过。QA-01 真人平台验收仍不勾选。未对 `prj_a63f79b2` 公开发布。

## REV-04 本轮五项复审修复（2026-09-10）

- [x] AUTO-01：已有主稿恢复返回持久正文；恢复前与交付前检查主稿及平台稿，失败保留人工修改。新增 3 个回归用例，关联专项 28 passed。
- [x] AUTO-02：账号级 OS 文件锁覆盖整个执行过程，进程退出自动释放；活跃任务不接管、终态不重跑、取消后不交付。恢复与并发专项 15 passed。AUTO-01 上项 commit `d226a26`。
- [x] AUTO-01：生图前校验定价并原子预留账号预算；结果未知保留预留与请求意图，不自动重复；已落盘图片恢复资产并幂等结算费用。图片与恢复专项 68 passed。AUTO-02 上项 commit `ff31dfa`。
- [x] DEL-02：按原始请求幂等键与查询回执链闭合发送意图；明确失败可显式重试，历史记录保持追加写；超时、缺回执、查询异常继续 unknown。发布专项 25 passed。预算上项 commit `5bd401d`。
- [x] QA：本轮 137 个测试文件分 10 批全量验证，**1950 passed**，前端生产构建通过。DEL-02 上项 commit `b578d52`。[验证记录](./product-validation/review-04-regression.md)。QA-01 的真人平台验收仍未完成。

### QA-01 真实流程用例交接（2026-09-10）

- [x] 编写 [T01–T08 真实流程验收用例](./product-validation/real-flow-acceptance.md)，包含个人创作、配图改图、版本审批、公众号草稿、多账号/头条、真实排期、恢复并发及质量预算。附故障模拟矩阵和报告格式。仅完成用例编写，未执行真实验收，不改变 QA-01 状态。

### UX-FIX-01 真实验收阻塞修复（2026-09-10）

- [x] 修复未绑定作品整页消失、账号选择前置及绑定失败/延迟响应保护、只填想法创建、窄屏侧栏遮挡。浏览器回归 10 passed，Python 分批全量 1950 passed，前端构建通过。真实页面完成新作品 `prj_c0d2c7df` 的创建、访谈、手工保存刷新；原作品只读核对。[局部复测与证据](./product-validation/ux-blockers-fixed/README.md)。未将局部通过记为 T01/T03/T05 全流程通过；平台交付及 QA-01 仍待验收。

  ✅ 完成于 2026-09-10，commit `4aa7148`，备注：三个阻塞修复及局部浏览器走查通过，原始验收报告与未跟踪证据保留。
