...
- 先做图文创作闭环。小红书、视频号、Bilibili、数字人后置。**本轮用户已批准**微信/头条多账号与按账号频率的运营开发，见 [docs/CREATOR_OPERATIONS_TASKS.md](docs/CREATOR_OPERATIONS_TASKS.md)。
...
- **本轮任务入口**：[docs/CREATOR_OPERATIONS_TASKS.md](docs/CREATOR_OPERATIONS_TASKS.md)。用户已批准易用性改造、账号档案、多账号、微信/头条草稿与（仅在账号明确授权后的）公开发布、以及按账号频率的 AI 运营。不要再用旧规划把这些当成禁止开发项。
- **R1–R9 已完成**；R0 代理黄金路径已用 `prj_a63f79b2` 走通但未勾选（作者署名未代答）。BYOK 与公众号草稿箱已提交；`media_id=T0wXMUxGNoEwkhzIN5QdEFUVn2YpxoR2v0avT8k5WaZcSVVU2xWPs1gXBBMpDmgA`，正文配图已用 `draft/update` 补上。**未公开发布。**
...
1. `docs/PRODUCT_RESET_PLAN.md` — 产品背景；本轮授权覆盖其中“多账号/真发布后置”
2. `docs/TASKS.md` — 指向本轮唯一入口，不得机械认领未勾选 R0 或遗留 M*
...
1. **本轮任务优先**：按 `docs/CREATOR_OPERATIONS_TASKS.md` 执行；不要机械领取旧 M* 或未勾选 R0。完成任务后勾选并追加 `✅ 完成于 <日期>，commit <sha>，备注 <一句话>`。真实公开发布仍需账号级明确授权。
