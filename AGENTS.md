...
- 2026-08-12 访谈已经修订旧首页：打开 `/` 直接是个人创作输入，主题、自己的想法和可选资料一次触发完整图文文章；“今天 / 灵感 / 项目”等退为次级入口。
- 个人创作和自动化创作是两条入口，但复用同一文章、版本、图片、平台和安全底座；AI 改动必须以批注和 diff 可审阅、可拒绝、可撤销，不能静默覆盖正文。
...
6. `docs/product-validation/2026-08-12-confirmed-product-tasklist.md` — 新阶段闸门、详细任务和浏览器验收；先检查 `DOC-02` 是否获用户批准
